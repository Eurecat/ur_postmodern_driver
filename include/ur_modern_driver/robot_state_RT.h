...
inline Vector6 toArray(const std::vector<double>& src){
  assert(src.size()==6);
...
private:
  ...
  Vector6 tool_vector_actual_; //Actual Cartesian coordinates of the tool: (x,y,z,rx,ry,rz), where rx, ry and rz is a rotation vector representation of the tool orientation
  ...
  Vector6 tool_vector_target_; //Target Cartesian coordinates of the tool: (x,y,z,rx,ry,rz), where rx, ry and rz is a rotation vector representation of the tool orientation
  Vector6 tcp_speed_target_; //Target speed of the tool given in Cartesian coordinates
  std::array<bool,64> digital_input_bits_; //Current state of the digital inputs. NOTE: these are bits encoded as int64_t, e.g. a value of 5 corresponds to bit 0 and bit 2 set high
  ...
  std::array<double,3> tool_accelerometer_values_; //Tool x,y and z accelerometer values (software version 1.7)
  ...
  template <size_t SIZE> void unpackVector(uint8_t * buf, int start_index, std::array<double,SIZE>& output);

  void unpackDigitalInputBits(int64_t data, std::array<bool,64> &output);
...
template <size_t SIZE> inline void RobotStateRT::unpackVector(uint8_t * buf, int start_index, std::array<double,SIZE> &output) {
  uint64_t q;
