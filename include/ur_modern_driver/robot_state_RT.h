...
class RobotStateRT {
  private:
	...
	std::vector<double> tool_vector_actual_;  // Actual Cartesian coordinates of the tool:
	                                          // (x,y,z,rx,ry,rz), where rx,
	// ry and rz is a rotation vector representation of the tool orientation
	std::vector<double> tcp_speed_actual_;  // Actual speed of the tool given in Cartesian
	                                        // coordinates
	std::vector<double> tcp_force_;         // Generalised forces in the TC
	std::vector<double> tool_vector_target_;  // Target Cartesian coordinates of the tool:
	                                          // (x,y,z,rx,ry,rz), where rx,
	// ry and rz is a rotation vector representation of the tool orientation
	std::vector<double> tcp_speed_target_;  // Target speed of the tool given in Cartesian
	                                        // coordinates
	std::vector<bool> digital_input_bits_;  // Current state of the digital inputs. NOTE:
	                                        // these are
	                                        // bits encoded as
	// int64_t, e.g. a value of 5 corresponds to bit 0 and bit 2 set high
	std::vector<double> motor_temperatures_;  // Temperature of each joint in degrees
	                                          // celsius
	...
	std::vector<double> tool_accelerometer_values_;  // Tool x,y and z accelerometer
	                                                 // values
