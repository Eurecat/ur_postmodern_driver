...
class RosWrapper {
	...
	RosWrapper(std::string host, int reverse_port)
		...
		if (use_ros_control_) {
			...
			double max_vel_change =
			    0.12;  // equivalent of an acceleration of 15 rad/sec^2
		...
		// Using a very conservative value as it should be set through the parameter
		// server
		...
		sprintf(buf, "Bounds for set_payload service calls: [%f, %f]", min_payload,
		        max_payload);
		print_debug(buf);
		...
		if (robot_.start()) {
			if (use_ros_control_) {
				ros_control_thread_ =
				    new std::thread(boost::bind(&RosWrapper::rosControlLoop, this));
				...
				rt_publish_thread_ =
				    new std::thread(boost::bind(&RosWrapper::publishRTMsg, this));
  ...
  private:
	...
	void
	goalCB(actionlib::ServerGoalHandle<control_msgs::FollowJointTrajectoryAction> gh) {
		std::string buf;
		print_info("on_goal");
		if (!robot_.sec_interface_->robot_state_->isReady()) {
			...
			if (!robot_.sec_interface_->robot_state_->isRealRobotEnabled()) {
				result_.error_string = "Cannot accept new trajectories: Robot is not "
				                       "enabled";
				gh.setRejected(result_, result_.error_string);
				print_error(result_.error_string);
				return;
		...
		if (robot_.sec_interface_->robot_state_->isEmergencyStopped()) {
			result_.error_code = -100;  // nothing is defined for this...?
			result_.error_string = "Cannot accept new trajectories: Robot is emergency "
			                       "stopped";
			gh.setRejected(result_, result_.error_string);
			print_error(result_.error_string);
			return;
		...
		if (robot_.sec_interface_->robot_state_->isProtectiveStopped()) {
			result_.error_code = -100;  // nothing is defined for this...?
			result_.error_string = "Cannot accept new trajectories: Robot is protective "
			                       "stopped";
			gh.setRejected(result_, result_.error_string);
			print_error(result_.error_string);
			return;
		...
		if (!validateJointNames()) {
			...
			result_.error_string =
			    "Received a goal with incorrect joint names: " + outp_joint_names;
			gh.setRejected(result_, result_.error_string);
			print_error(result_.error_string);
			return;
		}
		...
		if (!has_limited_velocities()) {
			result_.error_code = result_.INVALID_GOAL;
			result_.error_string = "Received a goal with velocities that are higher "
			                       "than " +
		...
		has_goal_ = true;
		std::thread(&RosWrapper::trajThread, this, timestamps, positions, velocities)
		    .detach();
	}

	void
	cancelCB(actionlib::ServerGoalHandle<control_msgs::FollowJointTrajectoryAction> gh) {
	...
	bool setIO(ur_msgs::SetIORequest& req, ur_msgs::SetIOResponse& resp) {
		...
		if (req.fun == 1) {
			...
			// According to urdriver.py, set_flag will fail if called to rapidly in
			// succession
	...
	bool has_limited_velocities() {
		actionlib::ActionServer<control_msgs::FollowJointTrajectoryAction>::Goal goal =
		    *goal_handle_.getGoal();
		for (unsigned int i = 0; i < goal.trajectory.points.size(); i++) {
			for (unsigned int j = 0; j < goal.trajectory.points[i].velocities.size();
			     j++) {
	...
	void rosControlLoop() {
		...
		realtime_tools::RealtimePublisher<geometry_msgs::TwistStamped> tool_vel_pub(
		    nh_, "tool_velocity", 1);
	...
	void publishRTMsg() {
		...
		ros::Publisher wrench_pub =
		    nh_.advertise<geometry_msgs::WrenchStamped>("wrench", 1);
		ros::Publisher tool_vel_pub =
		    nh_.advertise<geometry_msgs::TwistStamped>("tool_velocity", 1);
	...
	void publishMbMsg() {
		...
		ros::Publisher io_pub =
		    nh_.advertise<ur_msgs::IOStates>("ur_driver/io_states", 1);
		...
		while (ros::ok()) {
			...
			if (robot_.sec_interface_->robot_state_->isEmergencyStopped() or
				...
				if (robot_.sec_interface_->robot_state_->isEmergencyStopped() and
				    !warned) {
					print_error("Emergency stop pressed!");
				} else if (robot_.sec_interface_->robot_state_->isProtectiveStopped() and
				           !warned) {
